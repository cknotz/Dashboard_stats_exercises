---
title: "First steps in the R environment"
...
The previous tutorial on how to make graphs with `ggplot` hopped over quite a few smaller and larger details. In this tutorial, we make up for this and go over some important concepts and tools in the R environment. Think of this as a fundamental introduction to the R-language.
...
question("How flexible is R with languages?",
         answer("R is like a computer in some sci-fi movie --- I can just talk to it like I would to a human being, and it totally gets the message!"),
...
question("How precise and careful do you have to be when you write code?",
         answer("R is quite smart and understands my code even if some commands are spelled wrong or there are missing commas"),
...
(OK, these questions were only half-serious...). Let's get to know more about the R language!

## The fundamentals of the R language {data-progressive=TRUE}

### The R vocabulary: Functions
...
Let's say we do some calculation in R --- e.g. `5*5` --- and we then want to use the result for another subsequent calculation.

If we do what we did above and simply calculate `5*5`, we get the result --- but it does not get stored. To store something in R, you use the *assignment operator*: `<-`

For example, if we wanted to store the value `25` for later use, we would *assign* it to an object that we define using the assignment operator `<-`. The code would be `result <- 25`. This means *"take the value of 25 and store it as 'result'"*.

If we want to see or use a stored object, we can simply type its name: `result`

Give it a try! Use the code above to store 25 as 'result'; below, type 'result' to print it out again:
```{r assign, exercise=T}
