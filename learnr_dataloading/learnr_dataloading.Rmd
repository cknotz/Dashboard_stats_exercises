...
An added benefit of this lesson is also that you get to know various different data sources and how you access them!
...
### Exercise 1: Download and read in a Stata dataset {data-progressive=TRUE}

This exercise should be easy to do. You just need to use the `haven`-package to read a Stata dataset into R. You can choose which dataset you want to work with. From here on, you should work in your own R-code file in RStudio.

First, load the `haven`-package --- if necessary, install it first. You install the package, as always, with `install.packages()`:
```{r insthav, echo=T, eval=F}
install.packages("haven")
```

###

Second, load the package with the `library()`-function:
```{r loadhav, echo=T, eval=F}
library(haven)
```

###

Now you need to get the link to the dataset of your choice. You can use the *Comparative Political Data Set*, the *Democracy Cross-National Data* or any other dataset you find interesting as long as it is in Stata (.dta) format. Go to the respective dataset website, copy the link and then use it in `haven`'s `read_dta()`-function:
```{r havexamp, echo=T, eval=F}
your_dataset <- read_dta("[link]")
```

If it worked, you should see your dataset listed in the *Environment* tab on the upper right-hand side in RStudio.

### Exercise 2: Using the `essurvey` package {data-progressive=TRUE}
