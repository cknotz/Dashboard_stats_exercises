...
```
All this code does is to create a `data.frame` (this is what R calls datasets) and save it as 'fruit'. We'll go over this in more detail later.
...
print.data.frame(data.frame(type = c("Apples","Oranges"),
                    number = c(42,17)))
```
Our dataset tells us, basically, that we have 42 apples and 17 bananas. Not more complicated than that.
...
Below, you find the *first piece* of the code necessary to create a scatterplot of these two variables. Run it to see what happens:

```{r cpds1, exercise=T}
ggplot(data = cpds, aes(x=womenpar,y=childcare))

# Note that the dataset is saved as 'cpds' -- short for Comparative Political Data Set
